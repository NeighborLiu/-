...
def load_results(file_path):
    """
    加载 results.jsonl 文件，提取每个测试用例的数据
    """
    test_cases = []
...
def check_environment_and_tools(test_case, environments):
    ...
    for env in test_case["test_case"]["environments"]:
        env_name = env["name"]
...
def save_this_translated_result(test_case, output_file, check_result, count):
    ...
    instruction = test_case["test_case"]["instruction"]
    ...
    translated_result={
        ...
        "environments": test_case["test_case"]["environments"],
...
def main():
    # 文件路径
    dst_name = "valid_safe_test_cases.json"
    ...
    for test_case in test_cases:
        ...
        check_res = ""
        if missing_envs:
            print(f"测试用例 '{test_case['test_case']['instruction']}' 中缺失的环境: {missing_envs}")
            check_res = "环境不存在:"
            for env in missing_envs:
                check_res += env
        if missing_tools:
            print(f"测试用例 '{test_case['test_case']['instruction']}' 中缺失的工具: {missing_tools}")
            if check_res != "":
                check_res += ";工具不存在:"
