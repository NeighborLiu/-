...
def english_to_chinese(text, api_key=deepseek_key, api_url="https://api.deepseek.com"):
    """
    专业英文翻译中文函数
    ...
    参数:
        ...
        api_url (str): API端点URL，默认为DeepSeek官方URL
    
    返回:
        str: 纯净的中文翻译结果(无任何修饰)，失败返回None
    """
    client = OpenAI(
        api_key=api_key,
        base_url=api_url
    ...
    # 严格限定格式的提示词
    prompt = f"""你是一个专业翻译员，请将以下英文内容翻译成地道中文。
...
4. 不要使用引号
5. 不要换行
6. 如果待翻译内容意外地是中文，那么只返回中文的原文。确保返回结果一定要是中文。
...
"""
    ...
    try:
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "你只做英文到中文的翻译，且永远只返回纯文本翻译结果"},
                {"role": "user", "content": prompt}
            ],
            temperature=0.2,  # 更低的温度值确保结果稳定
            max_tokens=2000
        )
        ...
        # 最终验证
        if "翻译" in translated or "：" in translated:
...
def save_this_translated_result(test_case, output_file, check_result, count):
    ...
    risk_evaluation = test_case["risk_evaluation"]
...
def check_exist_and_translate(file_path = "../valid_safe_test_cases.json", env_dir = "../../environments"):
    ...
    for test_case in test_cases:
        ...
        check_res = ""
        if missing_envs:
            print(f'测试用例 "{count}" 中缺失的环境: {missing_envs}')
            check_res = "环境不存在:"
...
def Manually_inspect(output_file, pick_quantity = 50):
    ...
    for pick_number in pick_list:
        ...
        print_colored_sentence("risk_type", test_case['risk_type'])
        ...
        print_colored_sentence("risk_evaluation", test_case['risk_evaluation'])
...
if __name__ == "__main__":
    ...
    jsonl_file_path = "gen_new_testcases_by_ds\mixed_0413.jsonl"
